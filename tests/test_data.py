...
from imagescry.data import ImageBatch, ImageFilesDataset
...
def test_image_batch_device() -> None:
    ...
    # Check the device
    check_functions.equal(image_batch.device, torch.device("cpu"))

    # Move to GPU
    if torch.cuda.is_available():
        image_batch = image_batch.to("cuda")
        check_functions.is_true(str(image_batch.device).startswith("cuda"))
        check_functions.is_true(image_batch.images.is_cuda)
        check_functions.is_true(image_batch.indices.is_cuda)
...
def test_sample_integer_size(variable_size_image_dataset: ImageFilesDataset) -> None:
    ...
    check_functions.is_instance(subset, Subset)

    # Check the size is correct
    check_functions.equal(len(subset), 5)

    # Check all indices are valid
    check_functions.is_true(all(0 <= idx < len(variable_size_image_dataset) for idx in subset.indices))
...
def test_sample_float_size(variable_size_image_dataset: ImageFilesDataset) -> None:
    ...
    check_functions.equal(len(subset), expected_size)

    # Test edge cases
    empty_subset = variable_size_image_dataset.sample(0.0)
    check_functions.equal(len(empty_subset), 0)

    full_subset = variable_size_image_dataset.sample(1.0)
    check_functions.equal(len(full_subset), len(variable_size_image_dataset))
...
def test_sample_seed_reproducibility(variable_size_image_dataset: ImageFilesDataset) -> None:
    ...
    check_functions.equal(subset1.indices, subset2.indices)

    # Sample with different seed
    subset3 = variable_size_image_dataset.sample(5, seed=1235)

    # Check indices are different
    check_functions.not_equal(subset1.indices, subset3.indices)
...
def test_dataloader_similar_shape_batcher(variable_size_image_dataset: ImageFilesDataset, max_batch_size: int) -> None:
    ...
    for batch in dataloader:
        ...
        check_functions.less_equal(len(batch), max_batch_size)
        ...
        check_functions.equal(num_unique_shapes, 1)
        ...
        check_functions.equal(expected_shapes, shapes)
    ...
    check_functions.equal(expected_image_indexes, observed_image_indexes)
...
def test_from_directory(tmp_path_factory: TempPathFactory) -> None:
    ...
    # Check dataset size
    check_functions.equal(len(dataset), len(shapes))
