...
def test_initialization() -> None:
    ...
    # Empty iterable
    points = Points([])
    check_functions.equal(len(points), 0)

    # List of Points
    points = Points([Point(1, 2), Point(3, 4)])
    check_functions.equal(len(points), 2)
    ...
    check_functions.equal(len(points), 4)


def test_magic_methods(multiple_points: Points) -> None:
    """Test core magic methods: __len__, __iter__, __repr__."""
    # Test __len__
    check_functions.equal(len(multiple_points), 5)

    # Test __iter__
    points_list = list(multiple_points)
    check_functions.equal(len(points_list), 5)
    check_functions.equal(points_list[0], Point(1, 2))

    # Test __repr__ with truncation
    repr_str = repr(multiple_points)
    check_functions.is_true("Points([" in repr_str)
    check_functions.is_true("Point(x=1, y=2)" in repr_str)
    check_functions.is_true("... (0 more)" not in repr_str)  # Should show all 5 points


def test_single_integer_indexing(multiple_points: Points) -> None:
    """Test single integer indexing."""
    # Test positive indexing
    check_functions.equal(multiple_points[0], Point(1, 2))
    check_functions.equal(multiple_points[2], Point(5, 6))

    # Test negative indexing
    check_functions.equal(multiple_points[-1], Point(9, 10))
    check_functions.equal(multiple_points[-2], Point(7, 8))
...
def test_slicing(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(sliced), 2)
    check_functions.equal(sliced[0], Point(3, 4))
    check_functions.equal(sliced[1], Point(5, 6))

    # Test step slicing
    sliced = multiple_points[::2]
    check_functions.equal(len(sliced), 3)
    check_functions.equal(sliced[0], Point(1, 2))
    check_functions.equal(sliced[1], Point(5, 6))

    # Test negative slicing
    sliced = multiple_points[-3:-1]
    check_functions.equal(len(sliced), 2)
    check_functions.equal(sliced[0], Point(5, 6))
    check_functions.equal(sliced[1], Point(7, 8))

    # Test empty slice
    sliced = multiple_points[1:1]
    check_functions.equal(len(sliced), 0)
...
def test_boolean_indexing(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(filtered), 3)
    check_functions.equal(filtered[0], Point(1, 2))
    check_functions.equal(filtered[1], Point(5, 6))
    check_functions.equal(filtered[2], Point(9, 10))
...
def test_fancy_indexing(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(selected), 0)

    # Test valid indices
    indices = [0, 2, 4]
    selected = multiple_points[indices]
    check_functions.equal(len(selected), 3)
    check_functions.equal(selected[0], Point(1, 2))
    check_functions.equal(selected[1], Point(5, 6))
    check_functions.equal(selected[2], Point(9, 10))
    ...
    check_functions.equal(len(selected), 3)
    check_functions.equal(selected[0], Point(1, 2))
    check_functions.equal(selected[1], Point(1, 2))
    check_functions.equal(selected[2], Point(5, 6))
...
def test_append(empty_points: Points) -> None:
    ...
    check_functions.equal(len(empty_points), 1)
    check_functions.equal(empty_points[0], Point(1, 2))
...
def test_extend(empty_points: Points) -> None:
    ...
    check_functions.equal(len(empty_points), 2)
    check_functions.equal(empty_points[0], Point(1, 2))
    check_functions.equal(empty_points[1], Point(3, 4))
...
def test_batch(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(batches), 3)
    check_functions.equal(len(batches[0]), 2)
    check_functions.equal(len(batches[1]), 2)
    check_functions.equal(len(batches[2]), 1)

    # Test batching with size > len
    batches = multiple_points.batch(10)
    check_functions.equal(len(batches), 1)
    check_functions.equal(len(batches[0]), 5)

    # Test batching with size = 1
    batches = multiple_points.batch(1)
    check_functions.equal(len(batches), 5)
    check_functions.equal(len(batches[0]), 1)
...
def test_sort(multiple_points: Points) -> None:
    ...
    check_functions.equal(sorted_points[0], Point(1, 2))
    check_functions.equal(sorted_points[-1], Point(9, 10))

    # Test sorting in reverse
    sorted_points = multiple_points.sort(lambda p: p.x, reverse=True)
    check_functions.equal(sorted_points[0], Point(9, 10))
    check_functions.equal(sorted_points[-1], Point(1, 2))
...
def test_take(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(taken), 3)
    check_functions.equal(taken[0], Point(1, 2))
    check_functions.equal(taken[1], Point(5, 6))
    check_functions.equal(taken[2], Point(9, 10))

    # Test taking duplicate indices
    taken = multiple_points.take([0, 0, 2])
    check_functions.equal(len(taken), 3)
    check_functions.equal(taken[0], Point(1, 2))
    check_functions.equal(taken[1], Point(1, 2))
    check_functions.equal(taken[2], Point(5, 6))
...
def test_operation_chaining(multiple_points: Points) -> None:
    ...
    check_functions.equal(len(result), 2)
    check_functions.equal(result[0], Point(9, 10))
    check_functions.equal(result[1], Point(7, 8))
...
def test_batching_and_filtering(multiple_points: Points) -> None:
    ...
    batches = multiple_points.batch(2)
    check_functions.equal(len(batches), 3)  # Total of three batches

    # Filter each batch
    filtered_batches = [batch.filter(lambda p: p.x > 3) for batch in batches]
    check_functions.equal(len(filtered_batches[0]), 0)  # First batch has no points with x > 3
    check_functions.equal(len(filtered_batches[1]), 2)  # Second batch has two point with x > 3
    check_functions.equal(len(filtered_batches[2]), 1)  # Third batch has one point with x > 3
